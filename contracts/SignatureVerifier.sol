...
abstract contract SignatureVerifier {
    ...
    function checkMessageValidity(EncodedMessage memory _encodedMessage) internal view returns (bool) {
        require(_encodedMessage.target == address(this), "Target address doesn't match");
        require(_encodedMessage.blockNumber >= block.number, "Message is expired");
        return true;
    }
}
