...
import {
  ...
  Transfer,
} from '../../fsentities/generated/ShareInvites/ShareInvites';
...
export function createInviteAcceptedEvent(inviteId: BigInt, acceptedBy: Address): InviteAccepted {
  let inviteAcceptedEvent = changetype<InviteAccepted>(newMockEvent());

  inviteAcceptedEvent.parameters = new Array();

  inviteAcceptedEvent.parameters.push(new ethereum.EventParam('inviteId', ethereum.Value.fromUnsignedBigInt(inviteId)));
  inviteAcceptedEvent.parameters.push(new ethereum.EventParam('acceptedBy', ethereum.Value.fromAddress(acceptedBy)));

  return inviteAcceptedEvent;
}

export function createInviteDeclinedEvent(inviteId: BigInt, declinedBy: Address): InviteDeclined {
  let inviteDeclinedEvent = changetype<InviteDeclined>(newMockEvent());

  inviteDeclinedEvent.parameters = new Array();

  inviteDeclinedEvent.parameters.push(new ethereum.EventParam('inviteId', ethereum.Value.fromUnsignedBigInt(inviteId)));
  inviteDeclinedEvent.parameters.push(new ethereum.EventParam('declinedBy', ethereum.Value.fromAddress(declinedBy)));

  return inviteDeclinedEvent;
}

export function createInviteRevokedEvent(inviteId: BigInt, revokedBy: Address): InviteRevoked {
  let inviteRevokedEvent = changetype<InviteRevoked>(newMockEvent());

  inviteRevokedEvent.parameters = new Array();

  inviteRevokedEvent.parameters.push(new ethereum.EventParam('inviteId', ethereum.Value.fromUnsignedBigInt(inviteId)));
  inviteRevokedEvent.parameters.push(new ethereum.EventParam('revokedBy', ethereum.Value.fromAddress(revokedBy)));
...
export function createInviteSentEvent(
  ...
  inviteSentEvent.parameters.push(new ethereum.EventParam('from', ethereum.Value.fromAddress(from)));
  inviteSentEvent.parameters.push(new ethereum.EventParam('externalOrgOrTeamId', ethereum.Value.fromUnsignedBigInt(externalOrgOrTeamId)));
  inviteSentEvent.parameters.push(new ethereum.EventParam('sourceEntityId', ethereum.Value.fromUnsignedBigInt(sourceEntityId)));
  inviteSentEvent.parameters.push(new ethereum.EventParam('baseSharePerms', ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(baseSharePerms))));
  inviteSentEvent.parameters.push(new ethereum.EventParam('status', ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(status))));
