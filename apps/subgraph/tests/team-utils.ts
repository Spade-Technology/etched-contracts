...
export function createPermissionsUpdatedEvent(teamId: BigInt, account: Address, newPermission: i32): PermissionsUpdated {
  let permissionsUpdatedEvent = changetype<PermissionsUpdated>(newMockEvent());

  permissionsUpdatedEvent.parameters = new Array();

  permissionsUpdatedEvent.parameters.push(new ethereum.EventParam('teamId', ethereum.Value.fromUnsignedBigInt(teamId)));
...
export function createTeamCreatedEvent(teamId: BigInt, to: Address): TeamCreated {
  let teamCreatedEvent = changetype<TeamCreated>(newMockEvent());

  teamCreatedEvent.parameters = new Array();

  teamCreatedEvent.parameters.push(new ethereum.EventParam('teamId', ethereum.Value.fromUnsignedBigInt(teamId)));
  teamCreatedEvent.parameters.push(new ethereum.EventParam('to', ethereum.Value.fromAddress(to)));
...
export function createTransferToOrganisationEvent(teamId: BigInt, orgId: BigInt): TransferToOrganisation {
  let transferToOrganisationEvent = changetype<TransferToOrganisation>(newMockEvent());

  transferToOrganisationEvent.parameters = new Array();

  transferToOrganisationEvent.parameters.push(new ethereum.EventParam('teamId', ethereum.Value.fromUnsignedBigInt(teamId)));
  transferToOrganisationEvent.parameters.push(new ethereum.EventParam('orgId', ethereum.Value.fromUnsignedBigInt(orgId)));
