...
import { Approval, ApprovalForAll, EtchENSCreated, EtchTransferred, OwnershipTransferred, Transfer } from '../generated/Contract/Contract';
...
export function createEtchENSCreatedEvent(tokenId: BigInt, to: Address, name: string): EtchENSCreated {
  let etchEnsCreatedEvent = changetype<EtchENSCreated>(newMockEvent());

  etchEnsCreatedEvent.parameters = new Array();

  etchEnsCreatedEvent.parameters.push(new ethereum.EventParam('tokenId', ethereum.Value.fromUnsignedBigInt(tokenId)));
  etchEnsCreatedEvent.parameters.push(new ethereum.EventParam('to', ethereum.Value.fromAddress(to)));
  etchEnsCreatedEvent.parameters.push(new ethereum.EventParam('name', ethereum.Value.fromString(name)));

  return etchEnsCreatedEvent;
}

export function createEtchTransferredEvent(tokenId: BigInt, to: Address, name: string): EtchTransferred {
  let etchTransferredEvent = changetype<EtchTransferred>(newMockEvent());

  etchTransferredEvent.parameters = new Array();

  etchTransferredEvent.parameters.push(new ethereum.EventParam('tokenId', ethereum.Value.fromUnsignedBigInt(tokenId)));
  etchTransferredEvent.parameters.push(new ethereum.EventParam('to', ethereum.Value.fromAddress(to)));
  etchTransferredEvent.parameters.push(new ethereum.EventParam('name', ethereum.Value.fromString(name)));

  return etchTransferredEvent;
}
