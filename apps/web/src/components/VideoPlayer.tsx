...
export const VideoPlayer = ({ url }: { url: string }) => {
  ...
  return (
    <div className="flex w-full cursor-pointer flex-col items-center">
      <div className="relative" onClick={() => setPlaying(!playing)}>
        <ReactPlayer
          ref={playerRef}
          controls={false}
          playing={playing}
...
const Controls = ({ playerRef, playing, setPlaying, setMuted, setVolume }: ControlsProps) => {
  const seek = (seconds: number[]) => {
    if (seconds[0]) playerRef.seekTo(seconds[0], "seconds");
  };

  return (
    <div className="flex" style={{ width: playerRef.props.width }}>
      <button onClick={() => setPlaying(!playing)}>
        {playing ? <PauseCircleIcon className="h-6 w-6" /> : <PlayCircleIcon className="h-6 w-6" />}
      </button>
      <Slider value={[playerRef.getCurrentTime()]} max={Math.floor(playerRef.getDuration())} onValueChange={seek} />
      <button
        onClick={() => {
          setMuted(!playerRef.props.muted);
        }}
      >
        {playerRef.props.muted ? <VolumeXIcon className="h-6 w-6" /> : <Volume2Icon className="h-6 w-6" />}
      </button>
      <Slider
        style={{ width: 100 }}
...
const PlayIcon = () => (
  <svg width="94" height="94" viewBox="0 0 94 94" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <circle cx="47" cy="47" r="47" fill="white" fill-opacity="0.4" />
    <path
