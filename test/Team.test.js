...
describe("Team and Organization Contracts", function () {
    let Team, team, admin, paymaster, user1, user2, other;
    ...
    before(async function () {
        ...
        team = await Team.deploy(paymaster.address, admin.address);
        await team.deployed();
    });

    describe("Team Contract", function () {
        it("should set correct initial roles", async () => {
            const isAdmin = await team.hasRole(ADMIN_ROLE, admin.address);
            const isDefaultAdmin = await team.hasRole(DEFAULT_ADMIN_ROLE, admin.address);
        ...
        it("should allow default admin to add new admin", async () => {
            ...
            await team.connect(paymaster).grantRolebySignature(signature, ADMIN_ROLE, user1.address);
            const isAdmin = await team.hasRole(await team.ADMIN_ROLE(), user1.address);
            expect(isAdmin).to.be.true;
        });

        it("should not allow non-admin to add new admin", async () => {
            let signature = await generateSignature(user1, team.address);
            await expect(team.connect(paymaster).grantRolebySignature(signature, ADMIN_ROLE, user2.address)).to.be.revertedWith("Unauthorized");
        });

        it("should allow the admin to set default permissions", async () => {
            const perm = new BigNumber.from(2); // Permission.Write
            let signature = await generateSignature(admin, team.address);
            await team.setDefaultPermission(signature, user1.address, perm);
            const newPerm = await team.connect(admin).defaultPermissions(user1.address);
        ...
        it("should not allow non-admin to set default permissions", async () => {
            const perm = new BigNumber.from(2); // Permission.Write
            let signature = await generateSignature(user1, team.address);
            await expect(team.connect(user1).setDefaultPermission(signature, user2.address, perm)).to.be.revertedWith("Unauthorized");
        });

        it("should not allow third person to call contract to set default permissions", async () => {
            const perm = new BigNumber.from(2); // Permission.Write
            let signature = await generateSignature(admin, team.address);
            await expect(team.connect(user2).setDefaultPermission(signature, user2.address, perm)).to.be.revertedWith("Invalid sender");
        });

        it("should allow the admin to set default permissions from paymaster", async () => {
            const perm = new BigNumber.from(2); // Permission.Write
            let signature = await generateSignature(admin, team.address);
            await team.connect(paymaster).setDefaultPermission(signature, user2.address, perm);
            const newPerm = await team.defaultPermissions(user2.address);
            expect(newPerm.toString()).to.equal(perm.toString());
        });
        ...
        it("should not allow non-admin to set specific permissions", async () => {
            const tokenId = new BigNumber.from(1000);
            const perm = new BigNumber.from(2); // Permission.Write
            let signature = await generateSignature(user2, team.address);
            await expect(team.connect(paymaster).setPermission(signature, user2.address, tokenId, perm)).to.be.revertedWith("Unauthorized");
        });
        ...
        it("should allow secondary-admin to set specific permissions", async () => {
            ...
            const tokenId = new BigNumber.from(1000);
            const perm = new BigNumber.from(2); // Permission.Write
            signature = await generateSignature(user1, team.address);
            await team.connect(paymaster).setPermission(signature, user2.address, tokenId, perm);
            const newPerm = await team.permissions(user2.address, tokenId);
            expect(newPerm.toString()).to.equal(perm.toString());
        });
