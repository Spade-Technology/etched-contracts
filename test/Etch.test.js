...
async function etchMainTests({ teamContract, orgContract, etchContract, signers }) {
  ...
  describe("Team Owner Etch tests for creation and transfer (onlyowner)", () => {
    ...
    it("Should be able to transfer the team to another user, making the underlying Etch change ownership aswell.", async () => {
      ...
      await teamContract["safeTransferFrom(address,address,uint256)"](signers[0].address, signers[1].address, 1);
      ...
      await teamContract
        .connect(signers[1])
        ["safeTransferFrom(address,address,uint256)"](signers[1].address, signers[0].address, 1);
  ...
  describe("Team Permission (Read & Write, no transfer) Etch tests", () => {
    ...
    it("Should set Wallet 1 as read and write access for Team 3", async () => {
      await teamContract.setPermission(3, signers[1].address, ETeamPermissions.READWRITE);

      const access = await teamContract.hasPermission(3, signers[1].address, ETeamPermissions.READWRITE);

      await expect(access).to.equal(true);
    });

    it("Should set Wallet 2 as read access for Team 3", async () => {
      await teamContract.setPermission(3, signers[2].address, ETeamPermissions.READ);

      const access = await teamContract.hasPermission(3, signers[2].address, ETeamPermissions.READ);

      await expect(access).to.equal(true);
    });

    it("Both Wallet 2 and wallet 3 should not be able to set Permissions to wallet 4", async () => {
      await expect(teamContract.connect(signers[2]).setPermission(3, signers[3].address, ETeamPermissions.READ)).to.be.reverted;

      await expect(teamContract.connect(signers[3]).setPermission(3, signers[3].address, ETeamPermissions.READ)).to.be.reverted;
    });

    it("Wallet 1 should be able to mint a new Etch, but not Wallet 2 or 3", async () => {
      await etchContract.connect(signers[1]).safeMintForTeam(3, DocName, IPFSCid);

      await expect(etchContract.connect(signers[2]).safeMintForTeam(3, DocName, IPFSCid)).to.be.reverted;

      await expect(etchContract.connect(signers[3]).safeMintForTeam(3, DocName, IPFSCid)).to.be.reverted;
    });

    it("Wallet 2 and Wallet 3 should not be able to transfer the Etch to another team, or another user", async () => {
      await expect(etchContract.connect(signers[2]).transferToTeam(3, 2)).to.be.reverted;

      await expect(etchContract.connect(signers[3]).transferToTeam(3, 2)).to.be.reverted;

      await expect(etchContract.connect(signers[2]).transferFrom(signers[2].address, signers[3].address, 3)).to.be.reverted;

      await expect(etchContract.connect(signers[3]).transferFrom(signers[3].address, signers[2].address, 3)).to.be.reverted;
    });

    it("Both Wallet 1 and Wallet 2 should be able to read the Etch", async () => {
      const etch = await etchContract.connect(signers[1]).hasReadPermission(signers[1].address, 3);

      const etch2 = await etchContract.connect(signers[2]).hasReadPermission(signers[2].address, 3);

      await expect(etch).to.equal(etch2).to.equal(true);
    });

    it("Wallet 3 should not be able to read the Etch", async () => {
      const etch = await etchContract.connect(signers[3]).hasReadPermission(signers[3].address, 3);
  ...
  describe("Organisation Permission on Team", () => {
    it("Should be able to create a new Team & Etch using wallet 0", async () => {
      ...
      await etchContract.connect(signers[0]).safeMintForTeam(4, DocName, IPFSCid);
    ...
    it("Should be able to transfer the organisation to another user, making the underlying Team and Etch change ownership aswell.", async () => {
      ...
      // You have to pass a string, as there are two safeTransferFrom functions (one contains the data parameter)
      await orgContract["safeTransferFrom(address,address,uint256)"](signers[0].address, signers[1].address, 1);
    ...
    it("Set Wallet 1 as Admin access for Organisation 1", async () => {
      await orgContract.setPermission(1, signers[1].address, EOrgPermissions.ADMIN);
    ...
    it("Set Wallet 2 as Member access for Organisation 1 using Wallet 1", async () => {
      await orgContract.connect(signers[1]).setPermission(1, signers[2].address, EOrgPermissions.MEMBER);
    ...
    it("Wallet 2 should not be able to set Permissions to wallet 2", async () => {
      await expect(orgContract.connect(signers[2]).setPermission(1, signers[2].address, EOrgPermissions.MEMBER)).to.be.reverted;
    });
    ...
    it("Wallet 1 (admin) should be able to set team permissions for team 4", async () => {
      ...
      await teamContract.connect(signers[1]).setPermission(4, signers[1].address, ETeamPermissions.READWRITE);

      await teamContract.connect(signers[1]).setPermission(4, signers[2].address, ETeamPermissions.READ);
    });

    it("Wallet 2 should not be able to set team permissions for team 4", async () => {
      await expect(teamContract.connect(signers[2]).setPermission(4, signers[2].address, ETeamPermissions.READWRITE)).to.be
        .reverted;
    });

    it("Wallet 2 should be able to read the Etch, since Wallet 1 (admin) set the permissions to READ, but not READWRITE", async () => {
      await expect(await etchContract.connect(signers[2]).hasReadPermission(signers[2].address, 4)).to.equal(true);

      await expect(await etchContract.connect(signers[2]).hasWritePermission(signers[2].address, 4)).to.equal(false);
    });
  });
}
