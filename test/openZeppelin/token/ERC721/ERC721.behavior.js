...
function shouldBehaveLikeERC721(owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context("with minted tokens", function () {
      ...
      for (const { fnName, opts } of [
        ...
        describe(`via ${fnName}`, function () {
          ...
          describe("to a receiver contract that reverts with custom error", function () {
            it("reverts", async function () {
              const revertingReceiver = await ERC721ReceiverMock.new(RECEIVER_MAGIC_VALUE, RevertType.RevertWithCustomError);
              await expectRevertCustomError(
                this.token.methods[fnName + "(address,address,uint256)"](owner, revertingReceiver.address, tokenId, {
                  from: owner,
                }),
    ...
    describe("safe mint", function () {
      ...
      describe("via safeMint", function () {
        ...
        context("to a receiver contract returning unexpected value", function () {
          it("reverts", async function () {
            const invalidReceiver = await ERC721ReceiverMock.new("0x42", RevertType.None);
            await expectRevertCustomError(this.token.$_safeMint(invalidReceiver.address, tokenId), "ERC721InvalidReceiver", [
              invalidReceiver.address,
            ]);
          });
        });
        ...
        context("to a receiver contract that reverts with message", function () {
          it("reverts", async function () {
            const revertingReceiver = await ERC721ReceiverMock.new(RECEIVER_MAGIC_VALUE, RevertType.RevertWithMessage);
            await expectRevert(this.token.$_safeMint(revertingReceiver.address, tokenId), "ERC721ReceiverMock: reverting");
          });
        });
        ...
        context("to a receiver contract that reverts without message", function () {
          it("reverts", async function () {
            const revertingReceiver = await ERC721ReceiverMock.new(RECEIVER_MAGIC_VALUE, RevertType.RevertWithoutMessage);
            await expectRevertCustomError(this.token.$_safeMint(revertingReceiver.address, tokenId), "ERC721InvalidReceiver", [
              revertingReceiver.address,
            ]);
          });
        });
        ...
        context("to a contract that does not implement the required function", function () {
          it("reverts", async function () {
            const nonReceiver = await NonERC721ReceiverMock.new();
            await expectRevertCustomError(this.token.$_safeMint(nonReceiver.address, tokenId), "ERC721InvalidReceiver", [
              nonReceiver.address,
            ]);
          });
        });
    ...
    describe("approve", function () {
      ...
      context("when the sender does not own the given token ID", function () {
        it("reverts", async function () {
          await expectRevertCustomError(this.token.approve(approved, tokenId, { from: other }), "ERC721InvalidApprover", [other]);
        });
      });
  ...
  describe("_mint(address, uint256)", function () {
    it("reverts with a null destination address", async function () {
      await expectRevertCustomError(this.token.$_mint(ZERO_ADDRESS, firstTokenId), "ERC721InvalidReceiver", [ZERO_ADDRESS]);
    });
  ...
  describe("_burn", function () {
    it("reverts when burning a non-existent token id", async function () {
      await expectRevertCustomError(this.token.$_burn(nonExistentTokenId), "ERC721NonexistentToken", [nonExistentTokenId]);
    });
    ...
    describe("tokenByIndex", function () {
      ...
      [firstTokenId, secondTokenId].forEach(function (tokenId) {
        it(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
          ...
          const expectedTokens = [firstTokenId, secondTokenId, newTokenId, anotherNewTokenId].filter((x) => x !== tokenId);
  ...
  describe("metadata", function () {
    ...
    describe("token URI", function () {
      ...
      it("reverts when queried for non existent token id", async function () {
        await expectRevertCustomError(this.token.tokenURI(nonExistentTokenId), "ERC721NonexistentToken", [nonExistentTokenId]);
      });
