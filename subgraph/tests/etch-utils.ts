...
export function createCommentAddedEvent(
  ...
  commentAddedEvent.parameters.push(
    new ethereum.EventParam("comment", ethereum.Value.fromTuple(comment))
  )

  return commentAddedEvent
}

export function createEtchCreatedEvent(
  tokenId: BigInt,
  to: Address
): EtchCreated {
  ...
  etchCreatedEvent.parameters.push(
    new ethereum.EventParam(
      "tokenId",
      ethereum.Value.fromUnsignedBigInt(tokenId)
    )
...
export function createEtchTransferedToTeamEvent(
  tokenId: BigInt,
  from: Address,
  to: BigInt
): EtchTransferedToTeam {
  let etchTransferedToTeamEvent = changetype<EtchTransferedToTeam>(
    newMockEvent()
  )

  etchTransferedToTeamEvent.parameters = new Array()

  etchTransferedToTeamEvent.parameters.push(
    new ethereum.EventParam(
      "tokenId",
      ethereum.Value.fromUnsignedBigInt(tokenId)
    )
...
export function createInvididualPermissionsUpdatedEvent(
  tokenId: BigInt,
  account: Address,
  newPermission: i32
): InvididualPermissionsUpdated {
  let invididualPermissionsUpdatedEvent = changetype<
    InvididualPermissionsUpdated
  >(newMockEvent())
